...
public class XWindowManager {
	...
	public static boolean showDialog(Dialog dialog) {
		...
		if (dialog == null) {
			L.red("dialog is null");
			return false;
		}

		if (dialog.isShowing()) {
			L.red("dialog is showing");
			return false;
		}
	...
	/**
	 * 创建一个类似微信那样的底部弹出来的dialog
	 * @param context
	 * @param contentView
	 * @return
	 */
	public static Dialog createPopupDialog(Context context, View contentView) {
		Dialog dialog = new Dialog(context, R.style.XDialog);
		Window window = dialog.getWindow();
		WindowManager.LayoutParams windowParams = new WindowManager.LayoutParams();
		int[] size = getDeviceWidthAndHeight(context);
		windowParams.x = 0;
